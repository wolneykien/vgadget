...
int main(int argc, char **argv)
{
  ...
  if (0 > Res)
    {
      perror("[client] Error: unable to open an Interet family socket");
      close(SocketFD);
      exit(EXIT_FAILURE);
    }
  else if (0 == Res)
    {
      perror("[client] Error: first parameter does not contain a valid IP-address");
      close(SocketFD);
      exit(EXIT_FAILURE);
    }
  ...
  if ((in_fd = open(argv[3], O_RDONLY)) <= 0) {
	printf("[client] Can not open %s\n", argv[3]);
      }

      if (in_fd > 0) {
	if ((sent = sendfile(SocketFD, in_fd, 0, atoi(argv[4]))) < 0) {
	  perror("[client] Error: unable to sendfile");
	}
      }

      if (sent >= 0) {
	printf("[client] Sent %d bytes successfully\n", sent);
      }
